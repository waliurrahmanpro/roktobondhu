...
export async function fetchPublicDonorProfile(
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, user_id, full_name, blood_group, division, district, upazila, full_address, profile_picture_url, total_points, total_donations, reported_donations, donation_availability, verification_status, date_of_birth, next_eligible_date, is_banned, last_donation_date, created_at, updated_at"
    )
    .eq("user_id", userId)
    .maybeSingle();
