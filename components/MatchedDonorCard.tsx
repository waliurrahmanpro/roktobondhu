...
import type { MatchLogWithDonor } from "@/lib/data/matching";

type MatchedDonorCardProps = {
  match: MatchLogWithDonor;
  isLoggedIn: boolean;
  bloodGroup: string;
};
    ...
    <article className="rounded-2xl border border-green-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="relative h-12 w-12 overflow-hidden rounded-full border-2 border-red-100 bg-red-50">
      ...
      <dl className="mt-3 grid gap-1 text-sm">
        <div className="flex justify-between">
          ...
          <dd className="font-medium text-gray-900">
            {formatLocationLine({
              upazila: donor.upazila ?? undefined,
        ...
        <div className="flex justify-between">
          ...
          <dd className="font-medium text-amber-800">{donor.total_points ?? 0}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-gray-500">Match score</dt>
          <dd className="font-medium">{match.match_score}</dd>
        </div>
      </dl>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        ...
        <a
          ...
          className="inline-flex flex-col items-center justify-center gap-1 rounded-xl bg-red-600 px-2 py-2.5 text-xs font-semibold text-white hover:bg-red-700"
        >
        ...
        <a
          ...
          className="inline-flex flex-col items-center justify-center gap-1 rounded-xl bg-[#25D366] px-2 py-2.5 text-xs font-semibold text-white"
        >
